<?php
// filepath: c:\xampp\htdocs\mercado_admin\public\admin\vendedores.php
...
$lista = listarUsuariosPorRole($conn, 'vendedor', $q, $pagina, $pp);

$pageTitle = 'Vendedores';
$activeMenu = 'vendedores';
$topActions = [['label' => 'Novo vendedor', 'href' => 'vendedores_form']];
$subnavItems = [
    ['label' => 'Listar', 'href' => 'vendedores', 'active' => true],
    ['label' => 'Adicionar', 'href' => 'vendedores_form', 'active' => false],
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $id = (int)($_POST['id'] ?? 0);
    $novoAtivo = (int)($_POST['novo_ativo'] ?? 1);

    [$success, $msg] = atualizarStatusAtivoUsuario($conn, $id, $novoAtivo, ['vendedor']);
    $success ? $ok = $msg : $erro = $msg;
}
    ...
    <form method="get" class="mb-4 rounded-2xl border border-blackx3 bg-blackx/50 p-3 md:p-4">
      <div class="flex flex-col md:flex-row md:items-end md:flex-nowrap gap-3">
        ...
        <div class="hidden md:flex md:w-auto md:ml-auto items-center gap-2">
          ...
          <a href="vendedores" title="Limpar filtros" class="inline-flex items-center justify-center rounded-xl border border-blackx3 w-10 h-10 text-zinc-300 hover:border-greenx hover:text-white transition-all">
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="text-zinc-400 border-b border-blackx3">
            ...
            <th class="text-left py-3 pr-3">Status vendedor</th>
        ...
        <tbody>
            ...
            <tr class="border-b border-blackx3/50 hover:bg-blackx/40 transition">
              ...
              <td class="py-3 pr-3">
                <span class="px-2.5 py-1 rounded-full text-xs font-medium <?php
                  $sv = strtolower(trim($row['status_vendedor']));
                  echo $sv === 'aprovado' ? 'bg-greenx/15 border border-greenx/40 text-greenx' : ($sv === 'pendente' ? 'bg-orange-500/15 border border-orange-400/40 text-orange-300' : 'bg-red-500/15 border border-red-400/40 text-red-300');
                ?>">
                  <?= htmlspecialchars($row['status_vendedor']) ?>
                </span>
              </td>
              <td class="py-3 pr-3">
              ...
              <td class="py-3">
                <div class="flex items-center gap-2">
                  <a class="inline-flex items-center gap-1 rounded-lg bg-blackx border border-blackx3 hover:border-greenx px-2.5 py-1.5 text-xs text-zinc-300 hover:text-white transition" href="vendedores_form?id=<?= (int)$row['id'] ?>">
          ...
          <?php endforeach; ?>
          <?php if (!$lista['itens']): ?>
            <tr><td colspan="6" class="py-6 text-zinc-500">Nenhum vendedor encontrado.</td></tr>
          <?php endif; ?>
        </tbody>
      </table>
    </div>
...
(function () {
  function setVisual(btn, ativo) {
    ...
    const targetId = btn.dataset.statusTarget;
  ...
  document.addEventListener('click', async function (e) {
    ...
    try {
      const body = new URLSearchParams({
        id: btn.dataset.id,
        ativo: String(novo),
        scope: btn.dataset.scope || 'vendedores'
      });
      ...
      setVisual(btn, novo);
      toast(data.msg || 'Vendedor atualizado.');
    } catch (err) {
