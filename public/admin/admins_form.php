<?php
// filepath: c:\xampp\htdocs\mercado_admin\public\admin\admins_form.php
...
$id = (int)($_GET['id'] ?? 0);
$editar = $id > 0 ? obterUsuarioPorIdRole($conn, $id, 'admin') : null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($idPost > 0) {
        [$success, $msg] = atualizarUsuarioPainel($conn, $idPost, (string)$_POST['nome'], (string)$_POST['email'], 'admin', 'nao_solicitado', (string)($_POST['nova_senha'] ?? ''));
    } else {
        [$success, $msg] = criarUsuarioPainel($conn, (string)$_POST['nome'], (string)$_POST['email'], (string)$_POST['senha'], 'admin');
    }
...
$pageTitle = $editar ? 'Editar administrador' : 'Novo administrador';
$activeMenu = 'admins';
$subnavItems = [
    ['label' => 'Listar', 'href' => 'admins', 'active' => false],
    ['label' => 'Adicionar', 'href' => 'admins_form', 'active' => !$editar],
